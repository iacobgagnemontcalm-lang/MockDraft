...
function bench(label, opts, N=400) {
  let total=0;
  const fn=makeScorer(opts);
  for(let sim=0;sim<N;sim++){
    const rand=rng(sim*7919+1234),myTeamId=sim%NT;
  ...
  const avg=total/N;
  console.log((avg>=0?'+':'')+avg.toFixed(1).padStart(7)+' pts  '+label);
}

console.log('\n── capMult vs positiveMVOR investigation (400 sims each) ──\n');
bench('depth+steal, cap=0.8, no +mVOR  (old baseline)', {needAll:false,cliff:false,depth:true,run:false,demand:false,steal:true,capMult:0.8,positiveMVOR:false});
bench('depth+steal, cap=0.8, +mVOR     (current app)',  {needAll:false,cliff:false,depth:true,run:false,demand:false,steal:true,capMult:0.8,positiveMVOR:true});
